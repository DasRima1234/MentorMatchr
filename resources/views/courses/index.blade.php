                                    ...
                                    @foreach ($courses as $course)
                                        <tr>
                                            ...
                                            <td class="d-flex user-icon">
                                                <a href="javascript:void(0);" class="status-icon status-toggle-btn"
                                                    data-id="{{ $course->id }}">
                                                    <i class="fas fa-check-circle me-2 {{ $course->status == 'Active' ? 'text-success' : 'text-muted' }}" title="Current Status"></i>
                                                </a>

                                                <a href="{{ route('courses.edit', $course->id) }}" class="edit-icon"
                                                    title="Edit">
...
$(document).on('click', '.status-toggle-btn', function () {
    ...
    Swal.fire({
        title: 'Are you sure?',
        text: "This will update the course status.",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "{{ route('courses.toggleStatus') }}",
                type: "POST",
                data: {
                    course_id: courseId,
                    _token: "{{ csrf_token() }}"
